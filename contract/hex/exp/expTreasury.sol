...
/*
  expBank 기반 투표 거버넌스 + exp 트레저리 잠금 (즉시 실행 버전)

  요구사항
  - 투표권: expbank.user(voter).depo (스테이킹 수량)
  - 정족수: expbank.totalStaked() 기준 quorumBps
  - 과반 + 정족수 달성 시, 투표시간(endTime)이 남아도 즉시 execute 가능
  - 배포 시 입력값 최소화

  배포 시 필수 입력(3개)
  1) expToken        : exp 토큰 주소
  2) expBankAddress  : expBank 주소
  3) admin     : 출금 요청/수령 지갑
...
contract expTreasury is Ownable, ReentrancyGuard {
  IERC20 public immutable exp;
  IexpBank public immutable expBank;
  ...
  struct Proposal {
    uint256 id;
    uint256 amount;        // exp amount (정수 단위)
    uint256 startTime;
    uint256 endTime;
  ...
  constructor(
    address expToken,
    address expBankAddress,
    address initialadmin
  ) Ownable(msg.sender) {
    require(expToken != address(0), "exp: zero");
    require(expBankAddress != address(0), "HB: zero");
    require(initialadmin != address(0), "BEN: zero");

    exp = IERC20(expToken);
    expBank = IexpBank(expBankAddress);
    admin = initialadmin;
  ...
  // 트레저리 잠금 (예: 50,000,000 exp)
  // 사전에 exp.approve(this, amount) 필요
  function depositTreasury(uint256 amount) external onlyOwner nonReentrant {
    require(amount > 0, "amount=0");
    bool ok = exp.transferFrom(msg.sender, address(this), amount);
  ...
  function createWithdrawProposal(uint256 amount) external returns (uint256 id) {
    ...
    require(amount > 0, "amount=0");
    require(exp.balanceOf(address(this)) >= amount, "treasury insufficient");
  ...
  function vote(uint256 proposalId, bool support) external {
    ...
    require(weight > 0, "no stake in expbank");
    ...
    uint256 pend = 0;
    try expBank.pendingDividend(msg.sender) returns (uint256 v) {
      pend = v;
    } catch {}
  ...
  function execute(uint256 proposalId) external nonReentrant {
    ...
    require(exp.balanceOf(address(this)) >= p.amount, "insufficient");

    p.executed = true;

    bool ok = exp.transfer(admin, p.amount);
