...
contract expBank {
    ...
    struct UserInfo {
        ...
        // 추가 필드: 유저 평균 매수가/보유수량 추적
        uint256 netQty;       // 평균단가 계산 기준 보유수량(지갑+스테이킹 합)
        uint256 avgBuyPrice;  // 평균 매수가(HEX wei per token)
        uint256 totalPayHex;  // 총 매수 지불액 누적(HEX wei) - 참고용
        uint256 totalSellHex; // 총 환매 수령액 누적(HEX wei) - 참고용
    }
    ...
    event Bought(
        address indexed who,
        uint256 amount,          // 구매 기록(100% 기준)
        uint256 payHexWei,       // 지불 HEX
        uint256 autoStaked,      // 자동 스테이킹 수량
        uint256 received         // 지갑으로 받은 수량
    );
    ...
    constructor(address _exp, address _hutbank, uint8 _act) {
        require(_exp != address(0), "exp zero");
    ...
    function tokenInventory() public view returns (uint256) {
        return exp.balanceOf(address(this));
    }
    ...
    // 통합 대시보드 getter
    // - myQty: 지갑+스테이킹 합계
    // - currentPriceWei: 현재 price(0.01 + 잔고/스테이킹)
    // - myMarketCapWei: myQty * currentPriceWei
    // - myAvgBuyPriceWei: 내 평균매수가
    // - myPnlWei: (평가금액 - 원가) (음수 가능)
    // - myRoiBps_: 손익률(BPS). 1234=12.34%, -500=-5.00%
        ...
        )
    {
        ...
        uint256 wallet = exp.balanceOf(who);
        uint256 depo = u.depo;
        myQty = wallet + depo;
        ...
        uint256 costWei = myQty * myAvgBuyPriceWei;
    ...
    function myMarketCap(address who) external view returns (uint256) {
        UserInfo storage u = userInfo[who];
        uint256 qty = exp.balanceOf(who) + u.depo;
        return qty * price;
    }
    ...
    // price = 0.01 + (hexBalance / totalStaked)
    function priceUp() public returns (uint256) {
        ...
        if (totalStaked == 0) {
            ...
            uint256 dynamicPart = hexBalance() / totalStaked;
    ...
    function buy(uint256 amount, uint256 maxPay) external returns (bool) {
        ...
        if (userReceive > 0) {
            require(exp.transfer(msg.sender, userReceive), "token transfer fail");
        }
    ...
    function sell(uint256 amount) external returns (bool) {
        ...
        uint256 fee = (gross * 3) / 100; // 3%
        uint256 recvPay = gross - fee;

        require(exp.allowance(msg.sender, address(this)) >= amount, "allowance low");
        require(exp.transferFrom(msg.sender, address(this), amount), "token transfer fail");
    ...
    function withdraw() external returns (bool) {
        ...
        require(exp.transfer(msg.sender, amount), "token transfer fail");
    ...
    // 배당 단가: hexBalance/totalStaked (BASE_PRICE 미포함)
    function pendingDividend(address who) public view returns (uint256) {
        UserInfo storage u = userInfo[who];
        if (u.depo == 0) return 0;
        if (totalStaked == 0) return 0;

        uint256 _allow = hexBalance() / totalStaked; // HEX wei per token
