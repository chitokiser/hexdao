...
// 다른 bank들이 Hutbank에 수익(기록)을 올려주는 인터페이스
...
/*
    ...
    외부 수익:
    - 다른 bank들이 이 계약의 totalfeeup(amount)를 호출해서 totalfee 기록만 올림
    - 가격 갱신은 거래/클레임에서만 수행


*/
contract Hutbank is IHutBankFee {
    /* ========== Tokens ========== */
    ...
    constructor(address _hut,uint8 _act) {
        require(_hut != address(0), "hut zero");
    

        hut = IERC20(_hut);

        hutbank = address(this);
        admin = msg.sender;
        act = _act;
    ...
    function totalfeeup(uint256 amount) public override {
        ...
        )
    {
        ...
        myActualQty = hut.balanceOf(who) + u.depo;
    ...
    function buy(uint256 amount, uint256 maxPay) external returns (bool) {
        ...
        if (userReceive > 0) {
            require(hut.transfer(msg.sender, userReceive), "token transfer fail");
        }
    ...
    function sell(uint256 amount) external returns (bool) {
        ...
        require(hut.allowance(msg.sender, address(this)) >= amount, "allowance low");
        require(hut.transferFrom(msg.sender, address(this), amount), "token transfer fail");

        require(hextoken.transfer(msg.sender, recvPay), "hex transfer fail");

        if (fee > 0) {
            // 대장주는 보통 hutbank = address(this)로 두므로, 자기 자신에게 보내도 무방
            require(hextoken.transfer(hutbank, fee), "fee transfer fail");
            totalfee += fee;

            // 자기 자신이면 totalfeeup을 다시 호출하면 중복 카운트 가능 → 호출 금지
            if (hutbank != address(this)) {
    ...
    function withdraw() external returns (bool) {
        ...
        require(hut.transfer(msg.sender, amount), "token transfer fail");
