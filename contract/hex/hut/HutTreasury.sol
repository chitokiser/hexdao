...
/*
  HutBank 기반 투표 거버넌스 + HUT 트레저리 잠금 (즉시 실행 버전)

  요구사항
  - 투표권: hutbank.user(voter).depo (스테이킹 수량)
  - 정족수: hutbank.totalStaked() 기준 quorumBps
  - 과반 + 정족수 달성 시, 투표시간(endTime)이 남아도 즉시 execute 가능
  - 배포 시 입력값 최소화

  배포 시 필수 입력(3개)
  1) hutToken        : HUT 토큰 주소
  2) hutBankAddress  : HutBank 주소
  3) beneficiary     : 출금 요청/수령 지갑
...
contract HutTreasuryGovByBankFast is Ownable, ReentrancyGuard {
  IERC20 public immutable HUT;
  IHutBank public immutable hutBank;
  ...
  struct Proposal {
    uint256 id;
    uint256 amount;        // HUT amount (정수 단위)
    uint256 startTime;
    uint256 endTime;
  ...
  event BeneficiaryChanged(address indexed prev, address indexed next);
  ...
  event ProposalExecuted(uint256 indexed id, uint256 amount, address indexed beneficiary);
  event ProposalCanceled(uint256 indexed id);

  constructor(
    address hutToken,
    address hutBankAddress,
    address initialBeneficiary
  ) Ownable(msg.sender) {
    require(hutToken != address(0), "HUT: zero");
    require(hutBankAddress != address(0), "HB: zero");
    require(initialBeneficiary != address(0), "BEN: zero");

    HUT = IERC20(hutToken);
    hutBank = IHutBank(hutBankAddress);
  ...
  function vote(uint256 proposalId, bool support) external {
    ...
    require(weight > 0, "no stake in hutbank");
    ...
    uint256 pend = 0;
    try hutBank.pendingDividend(msg.sender) returns (uint256 v) {
      pend = v;
    } catch {}
  ...
  function execute(uint256 proposalId) external nonReentrant {
    ...
    require(HUT.balanceOf(address(this)) >= p.amount, "insufficient");

    p.executed = true;

    bool ok = HUT.transfer(beneficiary, p.amount);
    require(ok, "transfer fail");

    emit ProposalExecuted(proposalId, p.amount, beneficiary);
  }
