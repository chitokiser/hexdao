...
contract dainaTreasury is Ownable, ReentrancyGuard {
  IERC20 public immutable daina;
  IdainaBank public immutable dainaBank;
  ...
  struct Proposal {
    uint256 id;
    uint256 amount;        // daina amount (정수 단위)
    uint256 startTime;
    uint256 endTime;
  ...
  constructor(
    address dainaToken,
    address dainaBankAddress,
    address initialadmin
  ) Ownable(msg.sender) {
    require(dainaToken != address(0), "daina: zero");
    require(dainaBankAddress != address(0), "HB: zero");
    require(initialadmin != address(0), "BEN: zero");

    daina = IERC20(dainaToken);
    dainaBank = IdainaBank(dainaBankAddress);
    admin = initialadmin;
  ...
  // 트레저리 잠금 (예: 50,000,000 daina)
  // 사전에 daina.approve(this, amount) 필요
  function depositTreasury(uint256 amount) external onlyOwner nonReentrant {
    require(amount > 0, "amount=0");
    bool ok = daina.transferFrom(msg.sender, address(this), amount);
  ...
  function createWithdrawProposal(uint256 amount) external returns (uint256 id) {
    ...
    require(amount > 0, "amount=0");
    require(daina.balanceOf(address(this)) >= amount, "treasury insufficient");
  ...
  // 투표권(가중치): dainaBank.user(who).depo
  function votingPower(address who) public view returns (uint256 depo) {
    (, , depo, , ) = dainaBank.user(who);
  }

  // 정족수 요구치: dainabank.totalStaked 기준
  function quorumRequired() public view returns (uint256) {
    uint256 ts = dainaBank.totalStaked();
  ...
  function vote(uint256 proposalId, bool support) external {
    ...
    require(weight > 0, "no stake in dainabank");
    ...
    uint256 pend = 0;
    try dainaBank.pendingDividend(msg.sender) returns (uint256 v) {
      pend = v;
    } catch {}
  ...
  function execute(uint256 proposalId) external nonReentrant {
    ...
    require(daina.balanceOf(address(this)) >= p.amount, "insufficient");

    p.executed = true;

    bool ok = daina.transfer(admin, p.amount);
  ...
  function treasuryBalance() external view returns (uint256) {
    return daina.balanceOf(address(this));
  }
}
