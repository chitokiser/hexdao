...
/*
    ...
    외부 수익:
    - 이 계약은 dainabank라서 (대장주 hutbank와 달리) 외부 bank의 totalfeeup 업로드는 보통 쓰지 않지만,
      구조 호환을 위해 family/totalfeeup은 유지.
    - 단, daina sell fee는 hutbank에 상납(transfer + hutbank.totalfeeup 기록 업로드)
*/
contract dainabank {
    /* ========== Tokens ========== */
    ...
    constructor(address _daina, address _hutbank, uint8 _act) {
        require(_daina != address(0), "daina zero");
        ...
        )
    {
        ...
        // 실제 보유(지갑 + 스테이킹)
        myActualQty = daina.balanceOf(who) + u.depo;
    ...
    function buy(uint256 amount, uint256 maxPay) external returns (bool) {
        ...
        if (userReceive > 0) {
            require(daina.transfer(msg.sender, userReceive), "token transfer fail");
        }
    ...
    function sell(uint256 amount) external returns (bool) {
        ...
        require(daina.allowance(msg.sender, address(this)) >= amount, "allowance low");
        require(daina.transferFrom(msg.sender, address(this), amount), "token transfer fail");
    ...
    function withdraw() external returns (bool) {
        ...
        require(daina.transfer(msg.sender, amount), "token transfer fail");
