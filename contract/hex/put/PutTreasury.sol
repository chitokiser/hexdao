...
/*
  putBank 기반 투표 거버넌스 + put 트레저리 잠금 (즉시 실행 버전)

  요구사항
  - 투표권: putbank.user(voter).depo (스테이킹 수량)
  - 정족수: putbank.totalStaked() 기준 quorumBps
  - 과반 + 정족수 달성 시, 투표시간(endTime)이 남아도 즉시 execute 가능
  - 배포 시 입력값 최소화

  배포 시 필수 입력(3개)
  1) putToken        : put 토큰 주소
  2) putBankAddress  : putBank 주소
  3) admin     : 출금 요청/수령 지갑
...
contract putTreasury is Ownable, ReentrancyGuard {
  IERC20 public immutable put;
  IputBank public immutable putBank;
  ...
  struct Proposal {
    uint256 id;
    uint256 amount;        // put amount (정수 단위)
    uint256 startTime;
    uint256 endTime;
  ...
  constructor(
    address putToken,
    address putBankAddress,
    address initialadmin
  ) Ownable(msg.sender) {
    require(putToken != address(0), "put: zero");
    require(putBankAddress != address(0), "HB: zero");
    require(initialadmin != address(0), "BEN: zero");

    put = IERC20(putToken);
    putBank = IputBank(putBankAddress);
    admin = initialadmin;
  ...
  // 트레저리 잠금 (예: 50,000,000 put)
  // 사전에 put.approve(this, amount) 필요
  function depositTreasury(uint256 amount) external onlyOwner nonReentrant {
    require(amount > 0, "amount=0");
    bool ok = put.transferFrom(msg.sender, address(this), amount);
  ...
  function createWithdrawProposal(uint256 amount) external returns (uint256 id) {
    ...
    require(amount > 0, "amount=0");
    require(put.balanceOf(address(this)) >= amount, "treasury insufficient");
  ...
  function vote(uint256 proposalId, bool support) external {
    ...
    require(weight > 0, "no stake in putbank");
    ...
    uint256 pend = 0;
    try putBank.pendingDividend(msg.sender) returns (uint256 v) {
      pend = v;
    } catch {}
  ...
  function execute(uint256 proposalId) external nonReentrant {
    ...
    require(put.balanceOf(address(this)) >= p.amount, "insufficient");

    p.executed = true;

    bool ok = put.transfer(admin, p.amount);
  ...
  function treasuryBalance() external view returns (uint256) {
    return put.balanceOf(address(this));
  }
}
