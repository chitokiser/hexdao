...
contract vetTreasury is Ownable, ReentrancyGuard {
  IERC20 public immutable vet;
  IvetBank public immutable vetBank;
  ...
  struct Proposal {
    uint256 id;
    uint256 amount;        // vet amount (정수 단위)
    uint256 startTime;
    uint256 endTime;
  ...
  constructor(
    address vetToken,
    address vetBankAddress,
    address initialadmin
  ) Ownable(msg.sender) {
    require(vetToken != address(0), "vet: zero");
    require(vetBankAddress != address(0), "HB: zero");
    require(initialadmin != address(0), "BEN: zero");

    vet = IERC20(vetToken);
    vetBank = IvetBank(vetBankAddress);
    admin = initialadmin;
  ...
  // 트레저리 잠금 (예: 50,000,000 vet)
  // 사전에 vet.approve(this, amount) 필요
  function depositTreasury(uint256 amount) external onlyOwner nonReentrant {
    require(amount > 0, "amount=0");
    bool ok = vet.transferFrom(msg.sender, address(this), amount);
  ...
  function createWithdrawProposal(uint256 amount) external returns (uint256 id) {
    ...
    require(amount > 0, "amount=0");
    require(vet.balanceOf(address(this)) >= amount, "treasury insufficient");
  ...
  function vote(uint256 proposalId, bool support) external {
    ...
    require(weight > 0, "no stake in vetbank");
    ...
    uint256 pend = 0;
    try vetBank.pendingDividend(msg.sender) returns (uint256 v) {
      pend = v;
    } catch {}
  ...
  function execute(uint256 proposalId) external nonReentrant {
    ...
    require(vet.balanceOf(address(this)) >= p.amount, "insufficient");

    p.executed = true;

    bool ok = vet.transfer(admin, p.amount);
