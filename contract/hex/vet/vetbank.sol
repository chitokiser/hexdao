...
/*const href = `tokendetail.html?id=${encodeURIComponent(key)}`;
    vetbank.sol

    - vet token decimals: 0 (정수 토큰)
    ...
    외부 수익:
    - 이 계약은 vetbank라서 (대장주 hutbank와 달리) 외부 bank의 totalfeeup 업로드는 보통 쓰지 않지만,
      구조 호환을 위해 family/totalfeeup은 유지.
    - 단, vet sell fee는 hutbank에 상납(transfer + hutbank.totalfeeup 기록 업로드)
*/
contract vetbank {
    /* ========== Tokens ========== */
    ...
    constructor(address _vet, address _hutbank, uint8 _act) {
        require(_vet != address(0), "vet zero");
        ...
        )
    {
        ...
        // 실제 보유(지갑 + 스테이킹)
        myActualQty = vet.balanceOf(who) + u.depo;
    ...
    function buy(uint256 amount, uint256 maxPay) external returns (bool) {
        ...
        if (userReceive > 0) {
            require(vet.transfer(msg.sender, userReceive), "token transfer fail");
        }
    ...
    function sell(uint256 amount) external returns (bool) {
        ...
        require(vet.allowance(msg.sender, address(this)) >= amount, "allowance low");
        require(vet.transferFrom(msg.sender, address(this), amount), "token transfer fail");
    ...
    function withdraw() external returns (bool) {
        ...
        require(vet.transfer(msg.sender, amount), "token transfer fail");
